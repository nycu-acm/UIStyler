...
def main():
    ...
    parser.add_argument('--task', default='BUSBRA', help='task or dataset name')
    ...
    log_dir = os.path.join(opt.save_path, "logs.txt")
    ReDirectSTD(log_dir, "stdout", True)

    log_str = f"Data information: \n" \
                "Train data: {} \nVal data: {} \nTest data: {}\n"
    print(colored(log_str.format(opt.train_path, opt.val_path, opt.test_path), color="green", force_color=True))
    ...
    train_dataset = ImageToImage2D(opt.data_path, opt.train_path, tf_train, img_size=args.encoder_input_size)
    val_dataset = ImageToImage2D(opt.data_path, opt.val_path, tf_val, img_size=args.encoder_input_size)  # return image, mask, and filename
    ...
    test_dataset = ImageToImage2D(opt.data_path, opt.test_path, tf_test, img_size=args.encoder_input_size, class_id=1)  # return image, mask, and filename
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.eval_freq == 0:
            model.eval()
            dices, mean_dice, _, mean_iou, val_losses = get_eval(valloader, model, criterion=criterion, opt=opt, args=args)
            log_str = 'epoch [{}/{}], val loss: {:.4f}, dice: {:.4f}, iou: {:.4f}'.format(epoch, opt.epochs, val_losses, mean_dice, mean_iou)
            print(colored(log_str, color="yellow", force_color=True))
            if args.keep_log:
            ...
            if mean_dice > best_dice:
                ...
                save_path = opt.save_path + args.modelname + "_best"
                torch.save(model.state_dict(), save_path + ".pth", _use_new_zipfile_serialization=False)
                print(colored('save model to %s' % save_path, color="red", force_color=True))
                
                ##### Test the best model on the test set
                _, test_mean_dice, _, test_mean_iou, _ = get_eval(testloader, model, criterion=criterion, opt=opt, args=args)
                log_str = 'epoch [{}/{}], test dice: {:.4f}, test iou: {:.4f}'.format(epoch, opt.epochs, test_mean_dice, test_mean_iou)
                print(colored(log_str, color="green", force_color=True))
            ...
            log_str = (
                "======================================== \n"
                "Best model dice: {:.4f}, Best model iou: {:.4f}\n"
                "Test model dice: {:.4f}, Test model iou: {:.4f}\n"
                "========================================"
            ).format(best_dice, best_iou, test_mean_dice, test_mean_iou)
