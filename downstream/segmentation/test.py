...
def main():
    ...
    val_dataset = ImageToImage2D_Test(opt.data_path, args.source_test_dir, tf_val, img_size=args.encoder_input_size, class_id=1)  # return image, mask, and filename
...
#  ========================================================================= begin to evaluate the model ============================================================================
    ...
    if opt.mode == "train":
        dices, mean_dice, mean_hdis, mean_ious, val_losses = get_eval(valloader, model, criterion=criterion, opt=opt, args=args)
        print(f"mean dice: {mean_dice:.2f} \t mean iou: {mean_ious:.2f}")
        # print(f"mean hdis: {mean_hdis:.2f}")
    else:
