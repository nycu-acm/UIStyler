...
class ImageToImage2D_Test(Dataset):
    def __init__(self, dataset_path: str, split='train', joint_transform: Callable = None, img_size=256, prompt = "click", class_id=1,
        ...
        self.split = split
        id_list_file = split # os.path.join(dataset_path, '{0}.txt'.format(split))
        self.ids = [id_.strip() for id_ in open(id_list_file)]
        self.prompt = prompt
        self.img_size = img_size
        self.class_id = class_id
        self.class_dict_file = "./dataset/class.json"
    ...
    def __getitem__(self, i):
        ...
        img_name = os.path.basename(id_)
        img_path = os.path.join(os.path.join(self.dataset_path, img_name))
        root_folder = "/".join(self.split.split('/')[:-2])
        label_path = os.path.join(os.path.join(root_folder, id_)).replace('imgs', 'masks')
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        img_path = os.path.join(os.path.join(self.dataset_path, id_))
